...
| 명령당 상대·6관절 절대 안전 제한 | 계산·기록 | 계산·적용 |
...
로봇 포트와 손목·상단 카메라를 연결하고, 현재 두 스냅샷의 역할을 육안으로 확인해 5분 유효 세션 확인을 생성한 뒤 다음을 실행한다. 스트림을 열기 직전에 등록·PnP 목록·확인 유효기간을 다시 검사한다.
...
상단 보정이나 6개 관절 절대 범위가 아직 없어도 그림자 실행은 가능하지만 보고서에는 `motion_authorized=false`와 누락 항목이 남는다. 카메라 세션 확인은 그림자 스트림에도 필수다. 출력은 Git 비추적 `results/real_policy_shadow/<시각>/`에 저장한다.
...
그림자 모드는 다음 항목을 모두 만족해야 통과한다.

- 25주기 전체에서 `send_action`·Goal Position·기타 모터 쓰기 0회
- 손목·상단 프레임 누락 0회, 각각 기대 해상도 1280×720 확인
- ACT 출력과 단위 변환 전후 여섯 관절 값의 NaN/Inf 0회
- 사람이 관찰한 실제 관절 방향과 제안 방향의 부호가 여섯 관절 모두 일치
- 명령당 5도 상대 제한과 6개 관절 절대 작업 범위의 개입 기록이 기대대로 남음
- 카메라 역할, 색상, 크롭 방향, 블록·목표·자세 판정이 육안과 일치
- `actions.jsonl` 25행과 `summary.json`이 누락 없이 저장되고 모터 쓰기 수가 0으로 집계

하나라도 실패하면 관절 매핑의 `verified_on_physical_robot`를 바꾸거나 저속 능동 시험으로 넘어가지 않는다.

그림자 결과는 동작 방향과 소프트웨어 연결을 검사할 뿐 실제 집기 성공률을 뜻하지 않는다. 제안 관절 방향, 카메라 역할, 프레임 색상과 좌표가 정상임을 검수한 다음 관절 매핑 확인 절차와 저속 최초 픽으로 넘어간다.

## 능동 모드의 차단 조건

실제 명령 경로는 코드에 있지만 일반 실행 도구로 아직 노출하지 않았다. 다음 네 조건이 모두 충족되지 않으면 `RealSO101Adapter(mode="active")` 생성 단계에서 실패한다. 앞의 두 안전 게이트는 로컬 설정으로 비활성화할 수 없다.

1. 6개 관절 절대 범위와 같은 `layout_id`로 만든, 필수 검사가 모두 통과한 15분 이내 프리플라이트
2. 여섯 관절 매핑의 `verified_on_physical_robot=true`
3. 현재 카메라 장치 목록·index와 묶인 5분 이내 두 화면 육안 확인
4. 별도 능동 실행 확인 토큰 일치
